...
def run_reproject_pieline(las_file_path):
    las_filename = str(las_file_path.name)
    las_file = laspy.open(las_file_path) 
    if las_file.header.parse_crs().to_epsg() == 4326 :
        out_filename = f"{os.path.splitext(las_filename)[0]}_32634.las"
        out_filepath = os.path.join(os.path.dirname(las_file_path),out_filename)
        if os.path.exists(out_filepath):
            return None
        reproject_pipeline["pipeline"][0]["filename"] = str(las_file_path)
        reproject_pipeline["pipeline"][-1]["filename"] = str(out_filepath)
...
if __name__ == "__main__":  
    ...
    for filename1 in las_input_filenames:
        run_reproject_pieline(filename1)
